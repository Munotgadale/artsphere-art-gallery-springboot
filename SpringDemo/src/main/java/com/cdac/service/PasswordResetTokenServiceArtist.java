...
import com.cdac.repository.PasswordResetTokenRepositoryArtist;

@Service
public class PasswordResetTokenServiceArtist {

    @Autowired
    private PasswordResetTokenRepositoryArtist tokenRepositoryArtist;
    ...
    public PasswordResetTokenArtist createToken(Artist artist) {
        String token = UUID.randomUUID().toString();
        PasswordResetTokenArtist resetToken = new PasswordResetTokenArtist();
        resetToken.setToken(token);
        resetToken.setArtist(artist);
        resetToken.setExpiryDate(calculateExpiryDate(60));
        return tokenRepositoryArtist.save(resetToken);
    }
    public Artist findByEmail(String email) {
    	Optional<Artist> optionalUser = artistRepository.findByArtistEmail(email);
    	Artist existingUser = optionalUser.get();
    	return existingUser;
    }

    public PasswordResetTokenArtist findByToken(String token) {
        return tokenRepositoryArtist.findByToken(token);
    }
    ...
    public void sendPasswordResetEmail(Artist artist, String token) {
        String recipientAddress = artist.getArtistEmail();
        String subject = "Password Reset Request";
//        String resetLink = "http://your-app-url/reset-password?token=" + token;
        String originalEmail = artist.getArtistEmail();
        String originalPassword = artist.getArtistPassword();

        String message = "Dear " + artist.getArtistName() + ",\n\n"
	...
	public void sendLoginEmail(Artist artist) {
		String recipientAddress = artist.getArtistEmail();
	...
	public void sendDeactivationEmail(Artist artist) {
	    String recipientAddress = artist.getArtistEmail();
	...
	public void sendReactivationSuccessEmail(Artist artist) {
	    String recipientAddress = artist.getArtistEmail();
